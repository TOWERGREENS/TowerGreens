...
================================================================================
PART 6: SYSTEMS, LOGIC, AND INTEGRATIONS
================================================================================

--------------------------------------------------------------------------------
6.1 AUTHENTICATION — insforge.dev
--------------------------------------------------------------------------------

Provider: insforge.dev
Methods:
  Email/Password:
    Signup: POST to insforge.dev auth endpoint
    Login: POST to insforge.dev auth endpoint
    Returns: JWT token (stored in httpOnly cookie, NOT localStorage)
  Google OAuth:
    Redirect to insforge.dev Google OAuth flow
    Callback: insforge.dev creates/fetches user, sets cookie

Session management:
  Token stored in httpOnly cookie (secure, SameSite=Strict)
  Token refresh: handled automatically by insforge.dev SDK
  On logout: DELETE /api/auth/logout (clears server session) + clear client state

User roles stored in insforge.dev users table:
  "customer" → accesses /app/*
  "admin" → accesses /admin/*
  "rider" → accesses /rider/*

Route guards (frontend):
  React Router wrapper component checks auth state before rendering protected routes
  Unauthenticated → redirect to appropriate login
  Wrong role → redirect to own dashboard + toast "رسائی منع ہے"

Password requirements (enforced on signup and change):
  Minimum 8 characters
  At least 1 number
  No maximum (up to insforge.dev limits)

Session expiry:
  JWT expires: 7 days
  Refresh token: 30 days
  On expiry: redirect to login with toast "سیشن ختم — دوبارہ لاگ ان کریں"

--------------------------------------------------------------------------------
6.2 ORDER SYSTEM — COMPLETE FLOW
--------------------------------------------------------------------------------

ORDER STATES (and allowed transitions):

  placed → accepted (admin action)
  placed → rejected (admin action)
  accepted → preparing (admin action)
  preparing → out_for_delivery (admin action, requires rider assigned)
  out_for_delivery → delivered (rider action, after OTP verification)

  No backward transitions allowed (once accepted, cannot go back to placed, etc.)
  Rejected is terminal.
  Delivered is terminal.

ORDER ID FORMAT:
  #TG-[YYYYMMDD]-[4-digit sequential]
  Example: #TG-20260305-4821
  Sequential per day (resets at midnight PKT)
  Generated server-side at order creation

ORDER CREATION API — POST /api/orders/create:
  Request body:
    {
      items: [{ productId, quantity, unitPrice, nameUrdu }],
      deliveryAddress: { name, phone, address, city: "Lahore", note },
      paymentMethod: "jazzcash" | "cod",
      coinsApplied: 0 | 500 | 5000,
      jazzcashPhone: "03xxxxxxxxx" (if jazzcash)
    }
  Server actions:
    1. Validate all items are isAvailable = true
    2. Verify prices match current product prices (prevent client-side price tampering)
    3. If coinsApplied > 0:
       Verify user has >= coinsApplied coins
       Verify paymentMethod = "jazzcash" (coins not allowed on COD)
       Deduct coins: UPDATE users SET coinsBalance = coinsBalance - coinsApplied WHERE id = userId
       Create coinTransaction: { userId, amount: -coinsApplied, type: "deduct", orderId, reason: "order_discount" }
    4. Calculate final total:
       subtotal = sum(items[].unitPrice * items[].quantity)
       discount = coinsApplied (Rs value equals coin amount)
       finalTotal = max(0, subtotal - discount)
    5. Create order record in DB
    6. Generate 8-digit OTP (crypto.randomInt(10000000, 99999999).toString())
       Store in order.otp, order.otpExpiresAt = now + 24h
       NOTE: OTP is generated at order creation but only EMAILED to customer when order goes "out_for_delivery"
    7. Send push notification to admin: "نیا آرڈر آیا! #TG-XXXX"
    8. For COD orders:
       order.paymentStatus = "pending"
       order.orderStatus = "placed"
       Clear cart, redirect to /app/order-placed
    9. For JazzCash orders:
       order.paymentStatus = "awaiting"
       order.orderStatus = "pending_payment"
       Return order ID for payment initiation
       (order goes to "placed" status only after successful payment callback)

OTP EMAIL (sent when status → "out_for_delivery"):
  From: info@towergreens.site
  To: customer email
  Subject: "آپ کے آرڈر کا OTP — TowerGreens 🌱"
  Body (HTML email):
    TowerGreens logo header
    "سلام [Name]!"
    "آپ کا آرڈر #TG-XXXX راستے میں ہے!"
    Large OTP display:
      "آپ کا OTP: [12345678]"
      (JetBrains Mono, large, in a bordered box)
    Instruction:
      "یہ OTP رائیڈر کو دیں تاکہ ڈیلیوری مکمل ہو سکے۔"
      "اس OTP سے آپ کو TowerGreens Coins بھی ملیں گے — ڈیلیوری کے بعد اپنی ایپ کھولیں!"
    Footer: TowerGreens branding

EARN COINS ON ORDER (base rate):
  Triggered: when order status → "delivered" (NOT at order creation)
  Calculation:
    coinRate = settings["coin_earn_rate"] (e.g. 3 means 3%)
    baseCoins = Math.floor(order.totalAmount * coinRate / 100)
  Awarded: immediately on delivery, regardless of payment method
  Create coinTransaction: { userId, amount: +baseCoins, type: "earn", orderId, reason: "order_completion" }
  Update users.coinsBalance += baseCoins

EARN BONUS COINS (OTP verification — JazzCash only):
  Triggered: when customer successfully verifies OTP in coins popup
  Calculation:
    bonusCoins = Math.floor(order.totalAmount * 0.25)
    (25% of order value as coins — note: this is not a % of coins earned, it's % of Rs value)
    Example: Rs 500 order → 125 bonus coins
  Awarded: immediately on OTP verification
  Create coinTransaction: { userId, amount: +bonusCoins, type: "earn", orderId, reason: "otp_bonus" }
  Update users.coinsBalance += bonusCoins

COIN SPENDING RULES (enforced server-side, also client-side for UX):
  Can only spend: if paymentMethod = "jazzcash"
  Can only spend: if coinsBalance >= 500 (minimum threshold)
  Options:
    Spend 500 coins: subtract Rs 500 from order total
    Spend 5000 coins: subtract Rs 5000 from order total
    Cannot spend partial thresholds (e.g. cannot spend 300 coins)
    Cannot spend more than one threshold at once (cannot spend 500+5000=5500 coins in one order)
      Exception: if balance >= 5500, only the HIGHER threshold (5000) applies in a single order
  If order total < discount:
    Final total = Rs 0 (free order)
    Full threshold still deducted (no refund of remaining discount)
    Example: Rs 300 order + 500 coin spend → Rs 0 order, 500 coins deducted
  Coins deducted: at order creation (server-side, immediately)
  If order is later rejected: coins are REFUNDED
    PATCH /api/orders/[id]/reject → refund coinsApplied to user
    Create coinTransaction: refund entry

COD ORDERS — SPECIAL HANDLING:
  paymentStatus = "pending" (until admin manually marks as paid, or system marks on delivery)
  Order can proceed through all statuses same as JazzCash
  On delivery: admin should mark payment as received (optional admin action)
  No OTP coin bonus for COD (coins popup only triggers for JazzCash)
  Base coins ARE earned on COD delivery (same as JazzCash base rate)

--------------------------------------------------------------------------------
6.3 JAZZCASH INTEGRATION — COMPLETE 8-STEP SPEC
--------------------------------------------------------------------------------

STEP 1 — MERCHANT ACCOUNT:
  Create account at https://sandbox.jazzcash.com.pk (sandbox first)
  Obtain from JazzCash dashboard:
    Merchant ID (pp_MerchantID)
    Password (pp_Password)
    Integrity Salt (for HMAC)
  Production at: https://payments.jazzcash.com.pk
...
STEP 2 — ENVIRONMENT VARIABLES:
  ...
  JAZZCASH_PROD_URL=https://payments.jazzcash.com.pk/CustomerPortal/transactionmanagement/merchantform

STEP 3 — HMAC HASH GENERATION (SHA-256):
  All parameters to be hashed:
    pp_Amount, pp_BillReference, pp_Description, pp_Language, pp_MerchantID,
    pp_Password, pp_ReturnURL, pp_SubMerchantID (if applicable), pp_TxnCurrency,
    pp_TxnDateTime, pp_TxnExpiryDateTime, pp_TxnRefNo, ppmpf_1 through ppmpf_5

  Algorithm:
    1. Collect all parameters in an object
    2. Sort keys alphabetically
    3. Join values with '&': sortedValues = keys.sort().map(k => params[k]).join('&')
    4. Prepend salt: hashString = INTEGRITY_SALT + '&' + sortedValues
    5. Hash: HMAC-SHA256(INTEGRITY_SALT, hashString).hexDigest()
  
  Node.js implementation:
    const crypto = require('crypto');
    function generateHmac(params, salt) {
...
STEP 4 — /api/payment/initiate ENDPOINT:
  ...
  Request body:
    { orderId: "TG-20260305-4821", amount: 450, phone: "03001234567" }
  Server logic:
    1. Fetch order from DB, verify it belongs to current user, verify totalAmount matches
    2. Generate transaction reference: "T" + Date.now() + random4digits
    3. Format amount: JazzCash requires amount in paisa (Rs × 100), as string: "45000"
    4. Build JazzCash payload:
       pp_Version: "1.1"
       pp_TxnType: "MWALLET"
       pp_Language: "EN"
       ...
       pp_TxnExpiryDateTime: format(addMinutes(new Date(), 30), "yyyyMMddHHmmss")
       pp_TxnRefNo: txnReference
       pp_ReturnURL: process.env.JAZZCASH_RETURN_URL
       pp_Description: "TowerGreens Order " + orderId
       pp_SecureHash: generateHmac(payloadWithoutHash, process.env.JAZZCASH_INTEGRITY_SALT)
       ppmpf_1: userId
       ppmpf_2 through ppmpf_5: ""
    5. Store txnReference in order record (for callback verification)
    6. Return: { paymentUrl: JAZZCASH_URL, payload }
  
  Frontend on success:
    Build HTML form with all payload fields as hidden inputs
    Auto-submit form to paymentUrl (POST to JazzCash)
    Alternative: create the form dynamically in JS and submit

STEP 5 — /api/payment/callback ENDPOINT:
  Method: POST (JazzCash posts back)
  Route: /app/payment/callback (but this is handled server-side before React SPA loads)
  Logic:
    1. Extract all pp_* params from POST body
    2. Extract pp_SecureHash from params
    3. Rebuild hash without pp_SecureHash using same generateHmac
    4. Compare hashes (timing-safe compare: crypto.timingSafeEqual)
    5. If hash mismatch: log fraud attempt, redirect /app/payment?status=tampered
    6. Check pp_ResponseCode:
       "000" = success
       Others = failure (common codes: "121" = insufficient funds, "157" = expired, "167" = invalid OTP)
    7. Find order by pp_TxnRefNo or pp_BillReference
    8. If success:
       order.paymentStatus = "paid"
       order.paymentReference = pp_TxnRefNo
       order.orderStatus = "placed"
       Clear user's cart (DELETE /api/cart/all for this user)
       Push notification to customer: "آرڈر مل گیا! #TG-XXXX"
       Push notification to admin: "نیا ادا شدہ آرڈر! #TG-XXXX"
       Redirect: 302 to /app/order-placed?orderId=XXXX
    9. If failure:
       order.paymentStatus = "failed"
       order.orderStatus = "payment_failed" (terminal, user must retry with new order)
       Redirect: 302 to /app/payment?status=failed&orderId=XXXX&code=pp_ResponseCode

STEP 6 — FRONTEND FLOW:
  1. Customer selects JazzCash + enters phone number on checkout step 2
  2. Customer confirms order on step 3
  3. Frontend: POST /api/orders/create → get orderId
  4. Frontend: POST /api/payment/initiate { orderId, amount, phone } → get form data
  5. Frontend: dynamically creates hidden form, submits to JazzCash URL
  6. User: completes JazzCash payment on JazzCash's hosted page
  7. JazzCash: POST back to /app/payment/callback (server-side handler)
  8. Server: processes callback, redirects user to success or failure URL

STEP 7 — SANDBOX TESTING:
  Test credentials:
    Test merchant phone: 03001234567
    Test OTP (MPIN): 111222
    Use sandbox URL for all test transactions
  Test scenarios required before go-live:
    ✓ Successful payment → order placed correctly, callback verified
    ✓ Payment failure (insufficient funds) → order stays in payment_failed
    ✓ Tampered callback (wrong hash) → rejected, logged
    ✓ Coins applied correctly pre-payment
    ✓ Cart cleared after payment
    ✓ Notifications sent

STEP 8 — GO LIVE CHECKLIST:
  Replace JAZZCASH_API_URL with production URL
  Update .env with production MERCHANT_ID, PASSWORD, SALT
  Test one real transaction (minimum Rs 10)
  Verify callback receives data correctly
  Check notification delivery
  Remove sandbox-only test logs

--------------------------------------------------------------------------------
6.4 WEB PUSH NOTIFICATIONS — COMPLETE SETUP
--------------------------------------------------------------------------------
...
VAPID KEY GENERATION:
  ...
  Store in .env:
    ...
    VAPID_SUBJECT=mailto:info@towergreens.site

SERVICE WORKER (/public/sw.js):
  Events to handle:
    'push': receive push, show notification
    'notificationclick': handle tap → open specific URL
  ...
  Notification click handler:
    Navigate to data.url (order tracking URL, etc.)

  Register service worker in app:
    In main React entry point or useEffect:
      if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('/sw.js')
          .then(reg => setupPushSubscription(reg))
      }
...
PUSH SUBSCRIPTION STORAGE (insforge.dev):
  Table: pushSubscriptions
  Fields: id, userId, endpoint, p256dh, auth, createdAt, updatedAt

SENDING PUSH NOTIFICATIONS (server-side):
  Use: web-push npm package
  
  async function sendPushToUser(userId, payload) {
    const subscriptions = await db.pushSubscriptions.findAll({ where: { userId } });
...
NOTIFICATION PAYLOAD STRUCTURE:
  {
    title: "آرڈر ڈیلیوری پر ہے!",
    body: "آپ کا آرڈر #TG-4821 راستے میں ہے — تیار رہیں",
    url: "https://towergreens.site/app/orders/TG-4821",
    tag: "order-TG-4821",
    icon: "/icons/icon-192.png"
  }

MEDIAN.CO PUSH BRIDGE:
  median.co wraps the PWA into a native APK/iOS app
  It provides a native push notification bridge
  Configuration in median.co dashboard:
    Enable push notifications
    Set VAPID public key
    Configure icon and sound
  On native app: push arrives via OS-level notification (not browser push)
  Implementation is handled by median.co — no additional code required from us
  Just ensure: /sw.js is valid, VAPID setup is correct, PWA is installable

NOTIFICATION TRIGGERS (all with Urdu + English based on user language preference):

  Trigger                    | Title (Urdu)                | Body
  ─────────────────────────────────────────────────────────────────────
  Order placed (JazzCash)    | آرڈر موصول ہو گیا!          | آپ کا آرڈر #TG-XXXX موصول ہو گیا
  Order placed (COD)         | آرڈر موصول ہو گیا!          | آپ کا آرڈر #TG-XXXX موصول ہو گیا
  Order accepted             | آرڈر قبول ہو گیا! ✓         | ہم تیاری شروع کر رہے ہیں
  Order rejected             | آرڈر مسترد ہو گیا           | آپ کا آرڈر #TG-XXXX قبول نہیں ہوا
  Out for delivery           | آرڈر ڈیلیوری پر ہے! 🛵      | [rider name] آپ کی طرف آ رہے ہیں — OTP آپ کے ای میل پر بھیج دیا گیا
  Delivered                  | آرڈر پہنچ گیا! 🎉           | آپ کا آرڈر مکمل ہوا — Coins حاصل کریں!
  Admin: new order           | نیا آرڈر! #TG-XXXX          | Rs [amount] — [payment method]

GMAIL SMTP CONFIGURATION:
  Host: smtp.gmail.com
  Port: 587 (TLS) or 465 (SSL)
  User: info@towergreens.site
  Pass: App password (NOT regular Gmail password — enable 2FA + generate app password)
  From: "TowerGreens" <info@towergreens.site>
  
  Use nodemailer on server:
...
TABLE: users
  ...
  language:        ENUM('ur','en') DEFAULT 'ur'
...
TABLE: products
  ...
  nameEn:          TEXT
  descriptionUrdu: TEXT NOT NULL
  descriptionEn:   TEXT
  images:          TEXT[] (array of 4 image URLs)
  videoUrl:        TEXT
  ingredientsUrdu: TEXT
  calories:        INTEGER
  protein:         DECIMAL(5,2) (grams)
  fat:             DECIMAL(5,2)
  carbs:           DECIMAL(5,2)
  fiber:           DECIMAL(5,2)
  price:           INTEGER NOT NULL (in PKR, whole numbers)
  categoryId:      UUID FOREIGN KEY → categories.id
  isAvailable:     BOOLEAN DEFAULT true
  isFeatured:      BOOLEAN DEFAULT false
  orderCount:      INTEGER DEFAULT 0 (incremented on each order, for "popular" sort)
  createdAt:       TIMESTAMP DEFAULT NOW()
  updatedAt:       TIMESTAMP
...
TABLE: categories
  ...
  nameEn:          VARCHAR(100)
  slug:            VARCHAR(100) UNIQUE (URL-safe, kebab-case)
  icon:            TEXT (emoji or icon URL)
  sortOrder:       INTEGER DEFAULT 0
  createdAt:       TIMESTAMP

TABLE: orders
  id:              VARCHAR(30) (e.g. "TG-20260305-4821") PRIMARY KEY
  userId:          UUID FOREIGN KEY → users.id
  items:           JSONB (array: [{productId, nameUrdu, quantity, unitPrice}])
  subtotal:        INTEGER (sum of items before discount)
  coinsApplied:    INTEGER DEFAULT 0 (coins spent on this order)
  discount:        INTEGER DEFAULT 0 (Rs value of discount = coinsApplied)
  totalAmount:     INTEGER NOT NULL (final amount after discount, minimum 0)
  paymentMethod:   ENUM('jazzcash','cod') NOT NULL
  paymentStatus:   ENUM('pending','awaiting','paid','failed','refunded') DEFAULT 'pending'
  paymentReference: TEXT (JazzCash pp_TxnRefNo)
  orderStatus:     ENUM('pending_payment','placed','accepted','preparing','out_for_delivery','delivered','rejected','payment_failed')
  rejectionReason: TEXT
  deliveryAddress: JSONB ({ name, phone, address, city, note })
  riderId:         UUID FOREIGN KEY → users.id (rider user)
  otp:             VARCHAR(8) (8-digit number stored as string)
  otpExpiresAt:    TIMESTAMP (NOW + 24 hours, set at order creation)
  otpUsedAt:       TIMESTAMP (NULL until customer verifies in coins popup)
  coinsEarned:     INTEGER DEFAULT 0 (base coins earned on delivery)
  bonusCoinsEarned: INTEGER DEFAULT 0 (bonus from OTP verification)
  estimatedDelivery: INTEGER (minutes, from admin settings at time of order)
  createdAt:       TIMESTAMP DEFAULT NOW()
  updatedAt:       TIMESTAMP

TABLE: riders (extends users — riders are in users table with role='rider')
  No separate table needed. All rider data is in users table.
  Rider-specific view: users WHERE role = 'rider'
...
TABLE: notifications
  ...
  title:           TEXT NOT NULL
  body:            TEXT NOT NULL
  type:            ENUM('order','coins','promo','system') DEFAULT 'system'
  orderId:         VARCHAR(30) FOREIGN KEY → orders.id (nullable)
  isRead:          BOOLEAN DEFAULT false
  url:             TEXT (deep link: /app/orders/[id] etc.)
  createdAt:       TIMESTAMP DEFAULT NOW()
...
TABLE: coinTransactions
  ...
  type:            ENUM('earn','deduct','manual','refund')
  orderId:         VARCHAR(30) (nullable)
  reason:          TEXT (e.g. "order_completion", "otp_bonus", "order_discount", "manual_adjustment")
  createdAt:       TIMESTAMP DEFAULT NOW()

TABLE: cart
  id:              UUID
  userId:          UUID FOREIGN KEY → users.id UNIQUE (one cart per user)
  items:           JSONB (array: [{productId, nameUrdu, quantity, unitPrice, image}])
  updatedAt:       TIMESTAMP
...
TABLE: settings
  ...
  Default settings rows:
    ('delivery_duration_minutes', '35')
    ('coin_earn_rate', '3')              -- 3% of order value
...
TABLE: blogPosts
  ...
  contentUrdu:     TEXT (rich text / Markdown)
  contentEn:       TEXT
  coverImageUrl:   TEXT
...
--------------------------------------------------------------------------------
6.6 API ENDPOINTS — COMPLETE REFERENCE
--------------------------------------------------------------------------------

All API routes are under /api/*
Auth middleware on all routes except explicitly public ones
Roles enforced per endpoint

AUTH ENDPOINTS:
  POST /api/auth/signup             body: { name, email, phone, password, language }
  ...
  POST /api/auth/check-email        body: { email } → { available: bool }
  POST /api/auth/google             OAuth callback handler
  PATCH /api/auth/change-password   body: { currentPassword, newPassword }

USER ENDPOINTS:
  GET  /api/user/me                 returns full user profile
  PATCH /api/user/profile           body: { name, phone, language, profileImage }
  GET  /api/user/pending-otp-orders returns orders eligible for OTP coin claim
  GET  /api/user/coins              returns { balance, nextThreshold }
  GET  /api/user/saved-addresses    list of saved addresses
  POST /api/user/saved-addresses    add new address
  DELETE /api/user/saved-addresses/[id]

PRODUCT ENDPOINTS:
  GET  /api/products                query: category, sort, minPrice, maxPrice, available, page, limit
  GET  /api/products/[id]           single product with related products
  GET  /api/products/search         query: q, limit, page
  GET  /api/products/trending       returns popular search terms

CATEGORY ENDPOINTS:
  GET  /api/categories              all categories (sorted by sortOrder)
...
CART ENDPOINTS:
  ...
  DELETE /api/cart/all              clears entire cart

ORDER ENDPOINTS:
  GET  /api/orders                  current user's orders, query: status, page, limit
  GET  /api/orders/[id]             single order
  POST /api/orders/create           creates order (body per section 6.2)
  POST /api/orders/[id]/verify-otp  body: { otp } → validates and awards coins

PAYMENT ENDPOINTS:
  POST /api/payment/initiate        body: { orderId, amount, phone }
  POST /api/payment/callback        JazzCash callback (no auth — verifies HMAC)
...
NOTIFICATION ENDPOINTS:
  GET  /api/notifications           query: page, limit, unread=true
  GET  /api/notifications/unread-count
  PATCH /api/notifications/[id]/read
  PATCH /api/notifications/mark-all-read

MESSAGE ENDPOINTS:
  GET  /api/messages                user's message thread
  POST /api/messages                body: { message }

COINS ENDPOINTS:
  GET  /api/coins/transactions      query: type, page, limit

ADMIN ENDPOINTS (all require admin role):
  GET  /api/admin/dashboard/stats
  GET  /api/admin/analytics/revenue  query: period
  GET  /api/admin/analytics/products query: period
  GET  /api/admin/orders             query: status, paymentMethod, from, to, page, limit, q
  GET  /api/admin/orders/[id]
  PATCH /api/admin/orders/[id]/accept
  PATCH /api/admin/orders/[id]/reject  body: { reason }
  PATCH /api/admin/orders/[id]/status  body: { status }
  PATCH /api/admin/orders/[id]/assign-rider  body: { riderId }
  ...
  PATCH /api/admin/categories/reorder  body: { orderedIds: [] }
  GET  /api/admin/users              query: q, page, limit
  GET  /api/admin/users/[id]
  PATCH /api/admin/users/[id]/toggle-active
  POST /api/admin/coins/adjust       body: { userId, amount, reason }
  GET  /api/admin/riders
  POST /api/admin/riders/create      body: { name, email, password }
  PATCH /api/admin/riders/[id]/toggle-active
  PATCH /api/admin/riders/[id]/reset-password  body: { newPassword }
  POST /api/admin/notifications/broadcast  body: { title, body, target, userId? }
  GET  /api/admin/notifications/history
  GET  /api/admin/payments           query: method, status, from, to, page
  GET  /api/admin/messages           query: status (resolved/pending), page
  POST /api/admin/messages/[id]/reply  body: { reply }
  PATCH /api/admin/messages/[id]/resolve
  GET  /api/admin/settings
  PATCH /api/admin/settings          body: { updates: [{key, value}] }
  GET  /api/admin/content
  PATCH /api/admin/content           body: { heroLines, heroBanners, etc. }

RIDER ENDPOINTS (all require rider role):
  GET  /api/rider/orders             returns assigned orders for this rider
  GET  /api/rider/orders/[id]
  PATCH /api/rider/status            body: { isOnDuty }
  POST /api/rider/verify-otp         body: { orderId, otp }

--------------------------------------------------------------------------------
6.7 SSR CONFIGURATION — VITE SSR
--------------------------------------------------------------------------------

Vite is configured with TWO rendering modes:
  1. SSR for all /*, /about, /products, /veggies, /salads, /healthy-food,
     /chinese-style, /super-healthy, /processed-food, /how-it-works,
     /hydroponic-towers, /why-towergreens, /contact, /terms, /terms-en,
     /privacy, /privacy-en, /faq, /blog, /blog/[slug]
  2. SPA (no SSR) for /app/*, /admin/*, /rider/*

Vite SSR entry points:
  entry-client.jsx: hydrates SSR HTML on public routes
  entry-server.jsx: renders HTML string for public routes
  entry-spa.jsx: pure SPA entry for /app, /admin, /rider

Server (Node.js / Vercel Serverless Function):
  Intercept requests to public routes → run SSR render → return full HTML
  Intercept requests to /app/* → serve SPA HTML shell

EACH SSR PUBLIC PAGE must have:
  <title>[page title] — TowerGreens</title>
  <meta name="description" content="[unique per page]">
  <meta property="og:title" content="...">
  <meta property="og:description" content="...">
  <meta property="og:image" content="https://towergreens.site/og-image.jpg">
  <meta property="og:url" content="https://towergreens.site/[page-path]">
  <meta name="twitter:card" content="summary_large_image">
  <meta name="twitter:title" content="...">
  <meta name="twitter:description" content="...">
  <link rel="canonical" href="https://towergreens.site/[page-path]">
  <link rel="alternate" hreflang="ur" href="https://towergreens.site/[path]">
  <link rel="alternate" hreflang="en-PK" href="https://towergreens.site/[path]">
...
JSON-LD on homepage:
  {
    ...
    "name": "TowerGreens",
    "url": "https://towergreens.site",
    "email": "support@towergreens.site",
    "servesCuisine": "Healthy, Hydroponic Vegetables",
    "priceRange": "Rs 50 - Rs 2000",
    "address": { "@type": "PostalAddress", "addressLocality": "Lahore", "addressCountry": "PK" }
  }

JSON-LD on product pages (SSR):
  {
    "@context": "https://schema.org",
    "@type": "Product",
...
SITEMAP (/sitemap.xml — dynamically generated):
  Static public pages: all 20 public routes with <lastmod> and <priority>
  Homepage: priority 1.0, changefreq daily
  Category pages: priority 0.8, changefreq weekly
  Blog posts: generated from published blogPosts table (dynamic)
  Product detail pages (public /products/[id]): NOT in sitemap (SSR but app-gated)

Sitemap generated by: GET /api/sitemap → returns XML
Vercel cron or on-demand regeneration
...
All icons: #1B4332 green background, logo.png centered at 75% of icon area.
Generate using sharp npm package in a build script.
...
Favicon files in /public:
  ...
  apple-touch-icon.png (180x180, NO transparency — solid #1B4332 background)

iOS Splash Screens in /public/splash:
  apple-splash-640-1136.jpg    (iPhone SE)
  apple-splash-750-1334.jpg    (iPhone 8)
  apple-splash-828-1792.jpg    (iPhone XR)
  apple-splash-1170-2532.jpg   (iPhone 14)
  apple-splash-1284-2778.jpg   (iPhone 14 Pro Max)
  Design: #1B4332 green background, logo centered, Urdu tagline below

HTML meta tags in index.html:
  <meta name="apple-mobile-web-app-capable" content="yes">
  <meta name="apple-mobile-web-app-status-bar-style" content="black-translucent">
  <meta name="apple-mobile-web-app-title" content="TowerGreens">
  ...
  <link rel="apple-touch-startup-image" href="/splash/apple-splash-1170-2532.jpg" media="...">
  (add all splash sizes with appropriate media queries)
...
ROUTE PROTECTION:
  ...
  /rider/* — only accessible by users with role = "rider"
  /app/* — only accessible by authenticated users (any role)
    Exceptions (no auth required): /app/login, /app/signup, /app/forgot-password, /app/reset-password, /app/language-select

AUTH TOKEN:
  Store JWT in httpOnly cookie (not localStorage, not sessionStorage)
  Cookie: Secure=true, SameSite=Strict, HttpOnly=true
  Never expose JWT in any API response body
  Use CSRF token for all state-changing requests (POST/PUT/PATCH/DELETE)

INPUT VALIDATION (both client and server):
  All form inputs: sanitized server-side before DB write
  ...
  Never trust client-sent prices — always verify against DB prices server-side
  Max file size enforcement (5MB images, 50MB video) both client and server

RATE LIMITING:
  POST /api/auth/login: 5 attempts per IP per 15 minutes → 429 Too Many Requests
  POST /api/auth/signup: 10 per IP per hour
  POST /api/rider/verify-otp: 3 attempts per order
  POST /api/payment/initiate: 5 per user per hour
  POST /api/push/subscribe: 10 per user per day

JAZZCASH CALLBACK SECURITY:
  HMAC verification on every callback (mandatory, reject without this)
  Timing-safe compare for HMAC (prevent timing attacks)
  Log all callbacks (success, failure, tampered) with full params

OTP SECURITY:
  8-digit OTP generated with crypto.randomInt (cryptographically secure)
  Stored hashed in DB? No — stored plaintext is OK for 8-digit numeric OTP (low risk)
  24-hour expiry enforced server-side
  3-attempt lock enforced server-side
  OTP is single-use: otpUsedAt checked before accepting
  OTP tied to specific order (cannot use order A's OTP for order B)

HTTPS:
  Enforced everywhere via Vercel (automatic TLS)
  All redirects from HTTP → HTTPS

CORS:
  Allow: only towergreens.site and median.co iframe (if applicable)
  API: CORS restricted to same-origin + specific allowed origins

CONTENT SECURITY POLICY headers:
  default-src: 'self'
  script-src: 'self' 'nonce-[per-request]' cdn sources
  style-src: 'self' 'unsafe-inline' fonts.googleapis.com
  img-src: 'self' data: storage-domain
  font-src: 'self' fonts.gstatic.com
  connect-src: 'self' api.insforge.dev

ADMIN CREDENTIAL:
  admin@towergreens.site / quick@43_21aB
  Never expose in client-side code
  Change password recommended after first login

--------------------------------------------------------------------------------
6.11 MEDIAN.CO APK/IOS CONVERSION CHECKLIST
--------------------------------------------------------------------------------

Before submitting to median.co:

SPA FUNCTIONALITY:
  [ ] All /app/* pages fully functional as standalone SPA (no SSR dependency)
  [ ] React Router navigation works with no page reloads
  [ ] Deep links work: /app/product/[id], /app/orders/[id]
  [ ] Auth state persists across SPA navigation

TOUCH AND MOBILE:
  [ ] All interactive elements: minimum 48px × 48px touch targets
  [ ] No hover-only interactions (replace with tap/press)
  [ ] Swipe gestures work (cart item swipe-to-delete, image gallery swipe)
  [ ] No double-tap zoom issues (meta viewport correct)
  [ ] Overscroll behavior controlled (no bounce on Android)

PUSH NOTIFICATIONS:
  [ ] Service worker registered and functional
  [ ] Push notifications tested on mobile Chrome
  [ ] median.co push bridge configured in median.co dashboard
  [ ] Notification tap opens correct deep link

BOTTOM NAVIGATION:
  [ ] Bottom nav bar implemented and auto-hides correctly
  [ ] safe-area-inset-bottom applied (notch-safe on iPhone)
  [ ] Cart and notification badges update in real-time

ASSETS:
  [ ] manifest.json valid (check with Chrome DevTools → Application → Manifest)
  [ ] apple-touch-icon.png present at /apple-touch-icon.png
  [ ] All iOS splash screens present in /splash/
  [ ] All icon sizes present in /icons/
  [ ] theme-color meta tag present

PERFORMANCE:
  [ ] No console errors on mobile device (test on physical Android + iOS)
  [ ] Lighthouse PWA audit passes (90+)
  [ ] App installable (install prompt appears)
  [ ] App works offline for cached pages (service worker handles fetch)

MEDIAN.CO SPECIFIC:
  [ ] Configure median.co project: URL = https://towergreens.site/app
  [ ] Enable push notifications in median.co
  [ ] Set app icon (use icon-512.png)
  [ ] Set splash screen in median.co dashboard
  [ ] Configure in-app browser: open external links in in-app browser
  [ ] Test resulting APK on Android emulator + physical device

================================================================================
PART 7: SITE ARCHITECTURE — ALL 73 PAGES
================================================================================

PUBLIC LANDING (SSR, indexed, has header and footer):

  /                          Homepage (Section 1-9 + footer)
  /about                     About TowerGreens page
  /products                  All products public overview
  /veggies                   Vegetables category page
  /salads                    Salads category page
  /healthy-food              Healthy food category page
  /chinese-style             Chinese style dishes page
  /super-healthy             Super healthy category page
  /processed-food            Processed food category page
  /how-it-works              How hydroponics works (long explainer)
  /hydroponic-towers         The tower technology page
  /why-towergreens           Value proposition page
  /contact                   Contact form + support info
  /terms                     Terms & Conditions (Urdu)
  /terms-en                  Terms & Conditions (English)
  /privacy                   Privacy Policy (Urdu)
  /privacy-en                Privacy Policy (English)
  /faq                       Frequently Asked Questions
  /blog                      Blog index (list of posts)
  /blog/[slug]               Individual blog post

WEB APP SPA (auth-gated, NOT indexed, no header/footer, bottom nav):

  /app                       Entry point (redirects based on auth state)
  /app/home                  Main home screen
  /app/login                 Login screen
  /app/signup                Signup screen
  /app/forgot-password       Forgot password
  /app/reset-password        Reset password (token from email)
  /app/language-select       Language selection (first visit)
  /app/search                Search screen
  /app/products              All products listing
  /app/veggies               Vegetables filtered listing
  /app/salads                Salads filtered listing
  /app/healthy-food          Healthy food filtered listing
  /app/chinese-style         Chinese style filtered listing
  /app/super-healthy         Super healthy filtered listing
  /app/processed-food        Processed food filtered listing
  /app/product/[id]          Product detail
  /app/cart                  Shopping cart
  /app/checkout              Checkout (3-step)
  /app/payment               Payment status (failure screen)
  /app/payment/callback      JazzCash callback (server-handled, not UI screen)
  /app/order-placed          Order success celebration
  /app/orders                Order history list
  /app/orders/[id]           Single order tracking
  /app/profile               User profile
  /app/profile/edit          Edit profile
  /app/coins                 Coins wallet
  /app/notifications         Notifications list
  /app/direct-message        Support chat
  /app/terms                 T&C (language-aware)
  /app/privacy               Privacy policy (language-aware)

ADMIN PANEL (admin@towergreens.site only):

  /admin                     Entry (redirect to /admin/dashboard)
  ...
  /admin/delivery-settings   Delivery time settings
  /admin/payments            Payment records
  /admin/analytics           Analytics dashboard
  /admin/messages            Support messages inbox
  /admin/content             Content management
  /admin/settings            Site settings

RIDER PANEL (rider accounts only, created by admin):

  /rider                     Entry (redirect to /rider/dashboard)
  /rider/dashboard           Main dashboard
  /rider/orders              Order list
  /rider/orders/[id]         Single order + deliver button
  /rider/otp-verify          OTP entry screen
  /rider/language-select     Language toggle
  /rider/profile             Profile (read-only)

================================================================================
PART 8: QUALITY GATES — BEFORE LAUNCH
================================================================================
...
PERFORMANCE:
  ...
  [ ] 60fps animations (no jank on mid-range Android — test on Galaxy A-series)
  [ ] First API response < 500ms (target for home screen data load)
  [ ] Image optimization: all product images compressed to WebP, max 200KB each

FUNCTIONALITY (manual QA):
  [ ] Complete order flow: browse → cart → checkout (JazzCash) → payment → confirmation
  [ ] Complete order flow: browse → cart → checkout (COD) → confirmation
  [ ] OTP end-to-end:
      1. Order placed (JazzCash)
      2. Admin accepts, assigns rider, marks out-for-delivery
      3. OTP email received by customer
      4. Rider enters OTP on /rider/otp-verify → success
      5. Order marked delivered
      6. Customer opens app → coins popup appears
      7. Customer enters correct OTP → coins awarded, balance updates
  [ ] Coins threshold test — 500 coins:
      Create user with 500 coins
      Add items to cart (Rs 600 order)
      Select JazzCash, toggle 500 coins
      Verify total = Rs 100
      Verify 500 coins deducted after order
  [ ] Coins threshold test — free order (order < discount):
      Create user with 500 coins
      Add item (Rs 300 order)
      Apply 500 coins
      Verify total = Rs 0
      Verify 500 coins deducted
      Verify order placed as "free"
  [ ] Coins threshold test — 5000 coins:
      Same as 500-coin tests but with 5000 coins and Rs 5000 discount
  [ ] JazzCash sandbox: success case + failure case + HMAC tamper case
  [ ] Push notifications: order status changes trigger correct notifications
  [ ] Rider OTP wrong attempt (3 times) → locked
  [ ] Admin can unblock user
  [ ] Admin can create rider → rider can login → rider sees assigned orders
  [ ] Admin broadcast push notification → all users receive it

UI/UX (visual QA):
  [ ] All Urdu text: direction ltr, unicode-bidi plaintext — check every screen
  [ ] Dark theme correct on all 73 pages
  [ ] Skeleton loaders appear before data + crossfade correctly
  [ ] All animations at correct timing (no lag, no jump)
  [ ] Bottom nav auto-hide works on scroll
  [ ] Bottom nav safe-area on iPhone notch
  [ ] All error states styled correctly (inline errors, toast messages, empty states)
  [ ] Confetti on order-placed screen works
  [ ] OTP boxes: auto-advance, backspace, paste, shake-on-error
  [ ] Product detail: image swipe gallery + thumbnail strip
  [ ] Cart: swipe-to-delete on mobile
  [ ] Cart: undo delete (within 5s) works

SEO (verify on go-live):
  [ ] robots.txt accessible at https://towergreens.site/robots.txt
  [ ] All AI crawlers allowed (GPTBot, ClaudeBot, PerplexityBot)
  [ ] sitemap.xml generated and submitted to Google Search Console
  [ ] All public pages SSR-rendered (verify via curl or "view source")
  [ ] hreflang tags present on all public pages
  [ ] JSON-LD on homepage and product pages

SECURITY:
  [ ] No JWT in localStorage (must be in httpOnly cookie)
  [ ] HMAC verification on JazzCash callbacks (test with tampered callback)
  [ ] Rate limiting on login (5 attempts → blocked)
  [ ] Rate limiting on OTP submit (3 attempts → locked)
  [ ] Admin routes inaccessible by customer/rider accounts
  [ ] Rider routes inaccessible by customer/admin accounts
  [ ] No sensitive data logged to client console

MOBILE APP (median.co):
  [ ] All items in section 6.11 checklist completed
  [ ] APK installs and runs on Android 10+
  [ ] iOS build works on iOS 15+
...
Any deviation requires explicit written approval.
  ...
  1. DARK THEME IS DEFAULT EVERYWHERE.
  ...
  2. URDU DIRECTION IS ALWAYS LTR.
  ...
  3. LANDING PAGE HAS FOOTER. APP HAS NONE.
...
================================================================================
END OF TOWERGREENS PRD v4.0
================================================================================

Prepared by: TOWERGREENS Team
For: TowerGreens — Lahore, Pakistan
Handoff to: Antigravity IDE
Revision from: PRD v3.0
Total pages specified: 73
Total screens detailed: 35+ screen specs
All logic, flows, edge cases, CSS tokens, animations, APIs, DB schema included.
